...
const STORAGE_KEY = "aurora-loja-maconica-agenda-v2";
...
export default function AgendaPage() {
  ...
  useEffect(() => {
    try {
      ...
      setErro("Não foi possível ler a base local da agenda.");
    } finally {
      setCarregando(false);
    }
  }, []);
  ...
  const totalSolenidades = eventos.filter((item) => item.tipo === "Solenidade").length;
  const totalAniversarios = eventos.filter((item) => item.tipo === "Aniversário").length;
  ...
  function removerRegistro(id: string) {
    const confirmado = window.confirm("Deseja remover este evento da agenda?");
    if (!confirmado) return;

    setEventos((atual) => atual.filter((item) => item.id !== id));
  ...
  function salvarRegistro(evento: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const novoRegistro: EventoAgenda = {
        id: gerarId(),
        ...
        prioridade: form.prioridade.trim(),
        irmaoId: IRMAO_FIXO.id,
        irmaoNome: IRMAO_FIXO.nome,
        criadoEm: new Date().toISOString(),
      };

      setEventos((atual) => [novoRegistro, ...atual]);
      setForm(FORM_INICIAL);
      setMensagem("Evento salvo com sucesso na agenda protegida.");
    } catch {
      ...
      <section style={styles.container}>
        <header style={styles.hero}>
          <div style={styles.heroTop}>
            <div style={styles.heroBadge}>Agenda institucional e lembretes protegidos</div>
            <div style={styles.heroMiniBadge}>Aurora Loja Maçônica</div>
          </div>

          <div style={styles.heroGrid}>
            <div style={styles.heroMain}>
              <h1 style={styles.title}>Agenda e lembretes • Aurora Loja Maçônica</h1>

              <p style={styles.description}>
                Área criada para controlar reuniões, sessões, solenidades, aniversários, eventos e
                lembretes institucionais do irmão, com leitura clara, visual premium e base
                protegida para crescimento futuro.
              </p>
              ...
              <div style={styles.heroActions}>
                ...
                <Link href="/irmaos/documentos" style={styles.secondaryLink}>
                  Ir para documentos
                </Link>
              </div>
            ...
            <aside style={styles.heroAside}>
              <div style={styles.profileCard}>
                ...
                <div style={styles.profilePills}>
                  <span style={styles.pillDark}>Agenda</span>
                  <span style={styles.pillLight}>Protegida</span>
                  <span style={styles.pillLight}>Lembretes ativos</span>
                </div>
                ...
                <div style={styles.profileInfoGrid}>
                  ...
                  <div style={styles.profileInfoItem}>
                    ...
                    <strong style={styles.profileValue}>Agenda e lembretes institucionais</strong>
                  </div>
                </div>
              </div>
            </aside>
        ...
        <section style={styles.metricGrid}>
          <article style={styles.metricCard}>
            <span style={styles.metricLabel}>Eventos totais</span>
            <strong style={styles.metricValue}>{totalEventos}</strong>
            <span style={styles.metricHint}>Compromissos protegidos na agenda</span>
          </article>

          <article style={styles.metricCard}>
            <span style={styles.metricLabel}>Solenidades</span>
            <strong style={styles.metricValue}>{totalSolenidades}</strong>
            <span style={styles.metricHint}>Registros cerimoniais cadastrados</span>
          </article>

          <article style={styles.metricCard}>
            <span style={styles.metricLabel}>Aniversários</span>
            <strong style={styles.metricValue}>{totalAniversarios}</strong>
            <span style={styles.metricHint}>Datas importantes preservadas</span>
          </article>

          <article style={styles.metricCard}>
            <span style={styles.metricLabel}>Próximo compromisso</span>
            <strong style={styles.metricValueSmall}>
              {proximoEvento?.data ? formatarDataBR(proximoEvento.data) : "Sem agenda"}
            </strong>
        ...
        <section style={styles.grid}>
          <article style={styles.mainCard}>
            <div style={styles.cardHeader}>
              ...
              <div style={styles.cardBadge}>Salvar agenda</div>
            </div>
              ...
              <div style={styles.formGrid}>
                ...
                <label style={{ ...styles.field, gridColumn: "1 / -1" }}>
                  ...
                  <input
                    ...
                    placeholder="Ex.: Reunião, solenidade, lembrete, aniversário..."
                    style={styles.input}
                  />
                </label>
                ...
                <label style={styles.field}>
                  ...
                  <input
                    ...
                    placeholder="Ex.: Loja, templo, salão, endereço..."
                    style={styles.input}
                  />
                </label>
                ...
                <label style={{ ...styles.field, gridColumn: "1 / -1" }}>
                  ...
                  <textarea
                    ...
                    placeholder="Descreva o compromisso, os detalhes, observações e informações úteis."
                    rows={6}
          ...
          <aside style={styles.sideColumn}>
            <article style={styles.sideCard}>
              <div style={styles.cardHeader}>
                <div>
                  <h2 style={styles.cardTitle}>Agenda institucional</h2>
                  <p style={styles.cardSubtitle}>
                    Leitura protegida dos compromissos com ordenação cronológica.
                  </p>
                </div>

                <div style={styles.cardBadge}>{eventosFiltrados.length} exibidos</div>
              </div>
              ...
              <div style={styles.searchWrap}>
                <input
                  ...
                  placeholder="Buscar por tipo, título, local, prioridade, descrição..."
              ...
              <div style={styles.listWrap}>
                {carregando ? (
                  <div style={styles.emptyState}>Carregando agenda...</div>
                ) : eventosFiltrados.length === 0 ? (
                  <div style={styles.emptyState}>
                    Nenhum compromisso encontrado na base protegida.
                  </div>
                ) : (
                  eventosFiltrados.map((item) => (
                    <div key={item.id} style={styles.eventCard}>
                      <div style={styles.eventTop}>
                        <div>
                          <div style={styles.eventTitle}>{item.titulo}</div>
                          <div style={styles.eventTagRow}>
                            <span style={styles.eventTag}>{item.tipo}</span>
                            <span style={styles.priorityTag}>{item.prioridade}</span>
                          </div>
                        </div>
                      ...
                      <div style={styles.eventInfoGrid}>
                        ...
                        <div style={styles.eventInfoBox}>
                          <span style={styles.smallLabel}>Local</span>
                          <strong style={styles.smallValue}>{item.local || "Não informado"}</strong>
                        </div>

                        <div style={styles.eventInfoBox}>
                          <span style={styles.smallLabel}>Prioridade</span>
                          <strong style={styles.smallValue}>{item.prioridade}</strong>
                        </div>
                      ...
                      <div style={styles.obsBox}>
                        ...
                        <p style={styles.obsText}>
                          {item.descricao || "Sem descrição complementar neste compromisso."}
                        </p>
                      </div>
                    </div>
                  ))
            ...
            <article style={styles.sideCard}>
              <div style={styles.cardHeader}>
                <div>
                  ...
                  <p style={styles.cardSubtitle}>
                    Base pronta para reuniões, solenidades, aniversários e lembretes do irmão.
                  </p>
                </div>
              ...
              <div style={styles.readingStack}>
                <div style={styles.readingItem}>
                  <strong style={styles.readingTitle}>Agenda centralizada</strong>
                  <p style={styles.readingText}>
                    Esta área ajuda a preservar a rotina institucional em uma leitura clara, bonita
                    e organizada, com foco em compromissos e datas importantes.
                  </p>
                </div>
                ...
                <div style={styles.readingItem}>
                  ...
                  <p style={styles.readingText}>
                    Seguimos sua regra de ouro de manter botão de download para cópia local no PC ou
                    no celular quando possível.
                  </p>
                </div>
              </div>
            </article>
